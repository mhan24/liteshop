...
func TestOrderStateMachineFlow(t *testing.T) {
	...
	orderRec := orderdomain.Order{
		...
		Status:       orderdomain.OrderCreated,
		CreatedAt:    now,
		UpdatedAt:    now,
	}
	...
	if err := repo.AddLog(orderRec.ID, "order_created", "订单已创建", "", orderdomain.OrderCreated, 0); err != nil {
		t.Fatalf("log create: %v", err)
	}
	if err := repo.SetOrderStatusFrom(orderRec.ID, orderdomain.OrderCreated, orderdomain.OrderWaitingPayment); err != nil {
		t.Fatalf("transition to waiting: %v", err)
	}
	_ = repo.AddLog(orderRec.ID, "transaction_created", "BEpusdt 交易已创建", orderdomain.OrderCreated, orderdomain.OrderWaitingPayment, 0)
	...
	if o.Status != orderdomain.OrderWaitingPayment {
		t.Fatalf("status = %s, want %s", o.Status, orderdomain.OrderWaitingPayment)
	}
	...
	_ = repo.SetOrderStatus(o.ID, orderdomain.OrderDelivered)
	_ = repo.AddLog(o.ID, "payment_success", "支付成功", orderdomain.OrderWaitingPayment, orderdomain.OrderPaid, 0)
	_ = repo.AddLog(o.ID, "delivered", "卡密已发放", orderdomain.OrderPaid, orderdomain.OrderDelivered, 0)
	...
	if o2.Status != orderdomain.OrderDelivered {
...
func TestOrderCancelFreesCards(t *testing.T) {
	...
	repo := fixtures.NewOrderRepository(d)
	couponRepo := couponsqlite.NewCouponRepository(d)
	...
	if o.Status != orderdomain.OrderCancelled {
...
func TestFreeOrderWith100PercentCoupon(t *testing.T) {
	...
	repo := fixtures.NewOrderRepository(d)
	couponRepo := couponsqlite.NewCouponRepository(d)
	svc := orderapp.NewOrderService(repo, func(string) orderapp.PaymentGateway {
		t.Fatal("payFn must not be called for a free order")
		return nil
	}, nil)
	svc.SetCouponStore(couponRepo)
	if err := couponRepo.CreateCoupon(coupondomain.Coupon{Code: "FREE100", Type: "percent", Percent: 100, Active: true}); err != nil {
		t.Fatalf("create coupon: %v", err)
	}
	p := productdomain.Product{ID: productID, Name: "t", PriceCents: 100, MinQty: 1, MaxQty: 10, Status: "active"}
...
func TestRedeliverFromStock(t *testing.T) {
	...
	_ = inventorysqlite.NewInventoryRepository(d).ReleaseReservation(context.Background(), orderRec.ID)
	_ = repo.SetOrderStatus(orderRec.ID, orderdomain.OrderDeliveryFailed)
...
func TestOrderCountsWithTimezone(t *testing.T) {
	...
	defer d.Close()
	// 用 UTC 时区仓库
	repo := fixtures.NewOrderRepositoryWithTZ(d, time.UTC)
	// 插入一笔"今天"的订单（UTC 当天）
...
func TestCouponAndWholesale(t *testing.T) {
	...
	repo := fixtures.NewOrderRepository(d)
	couponRepo := couponsqlite.NewCouponRepository(d)
	svc := orderapp.NewOrderService(repo, func(string) orderapp.PaymentGateway { return bepusdt.NewClient("", "") }, func() settingsdomain.PaymentConfig { return settingsdomain.PaymentConfig{} })
	svc.SetCouponStore(couponRepo)

	// 固定券：满 1 元减 10 元（用于 1.8 元订单，可抵扣到 0 为止）
	if err := couponRepo.CreateCoupon(coupondomain.Coupon{Code: "TEST10", Type: "fixed", ValueCents: 1000, MinAmountCents: 100, MaxUses: 0, ProductID: 0, Active: true}); err != nil {
		t.Fatalf("create coupon: %v", err)
	}
	// 限购：少于 min_qty 应报错
	_, _, _, _, err = svc.CreateOrder(productdomain.Product{ID: pid, Name: "t", PriceCents: 100, MinQty: 2, MaxQty: 10}, 1, "a@b.com", "usdt-trc20", "bepusdt", "")
	if err == nil {
		t.Fatalf("qty below min should fail")
	}
	// 批发价：买 2 件单价 9 折 = 100*2*90/100 = 180
	// 固定券满 100 减 1000，抵扣后 180-1000 → 0 元订单直接完成（跳过支付）
	orderNo, _, discount, couponID, err := svc.CreateOrder(productdomain.Product{ID: pid, Name: "t", PriceCents: 100, MinQty: 2, MaxQty: 10, Wholesale: []productdomain.WholesaleTier{{MinQty: 2, Discount: 90}}}, 2, "a@b.com", "usdt-trc20", "bepusdt", "TEST10")
	if err != nil {
	...
	// 免费订单应已交付、卡密售出、券用量 +1
	c, _ := couponRepo.GetCouponByCode("TEST10")
	if c.UsedCount != 1 {
		t.Fatalf("coupon used = %d, want 1", c.UsedCount)
	}
	fo, _ := repo.GetOrderByNo(orderNo)
	if fo.Status != orderdomain.OrderDelivered {
	...
	if err := couponRepo.CreateCoupon(coupondomain.Coupon{Code: "TEST10B", Type: "fixed", ValueCents: 10, MinAmountCents: 100, MaxUses: 0, ProductID: 0, Active: true}); err != nil {
		t.Fatalf("create coupon B: %v", err)
	}
	orderNo2, _, _, couponID2, err := svc.CreateOrder(productdomain.Product{ID: pid, Name: "t", PriceCents: 100, MinQty: 2, MaxQty: 10, Wholesale: []productdomain.WholesaleTier{{MinQty: 2, Discount: 90}}}, 2, "a@b.com", "usdt-trc20", "bepusdt", "TEST10B")
	if err == nil {
	...
	// 支付失败后优惠券用量回滚
	c2, _ := couponRepo.GetCouponByCode("TEST10B")
	if c2.UsedCount != 0 {
...
func TestCouponConcurrentMaxUses(t *testing.T) {
	...
	defer d.Close()
	couponRepo := couponsqlite.NewCouponRepository(d)
	if err := couponRepo.CreateCoupon(coupondomain.Coupon{Code: "RACE", Type: "fixed", ValueCents: 50, MinAmountCents: 0, MaxUses: 1, ProductID: 0, Active: true}); err != nil {
		t.Fatalf("create coupon: %v", err)
	}
	c, _ := couponRepo.GetCouponByCode("RACE")
	if c.UsedCount != 0 {
	...
	for i := 0; i < 20; i++ {
		...
		go func() {
			defer wg.Done()
			if err := couponRepo.UseCoupon(c.ID, "ON"+fmt.Sprintf("%d", time.Now().UnixNano()), 50); err == nil {
				success <- 1
			}
		}()
	}
	...
	c2, _ := couponRepo.GetCouponByCode("RACE")
	if c2.UsedCount != 1 {
...
func TestOrderCostSnapshot(t *testing.T) {
	...
	if err := repo.CreatePendingOrder(&orderdomain.Order{
		...
		Status: orderdomain.OrderCreated, CreatedAt: now, UpdatedAt: now,
	}); err != nil {
...
func TestCouponRefundIdempotent(t *testing.T) {
	...
	defer d.Close()
	couponRepo := couponsqlite.NewCouponRepository(d)
	if err := couponRepo.CreateCoupon(coupondomain.Coupon{Code: "REF", Type: "fixed", ValueCents: 50, MinAmountCents: 0, MaxUses: 0, ProductID: 0, Active: true}); err != nil {
		t.Fatalf("create coupon: %v", err)
	}
	c, _ := couponRepo.GetCouponByCode("REF")
	if err := couponRepo.UseCoupon(c.ID, "REFORD", 50); err != nil {
		t.Fatalf("use coupon: %v", err)
	}
	c2, _ := couponRepo.GetCouponByCode("REF")
	if c2.UsedCount != 1 {
		t.Fatalf("used = %d, want 1", c2.UsedCount)
	}
	// 首次回滚：发生实际变化
	refunded, err := couponRepo.RefundByOrderNo("REFORD")
	if err != nil || !refunded {
		t.Fatalf("first refund should be real: %v %v", refunded, err)
	}
	c3, _ := couponRepo.GetCouponByCode("REF")
	if c3.UsedCount != 0 {
		t.Fatalf("used after refund = %d, want 0", c3.UsedCount)
	}
	// 再次回滚：幂等空操作
	refunded2, err := couponRepo.RefundByOrderNo("REFORD")
