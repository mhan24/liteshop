...
func mockTurnstileOK(t *testing.T) func() {
	t.Helper()
	old := turnstile.HTTPClient
	turnstile.HTTPClient = &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
	...
	return func() { turnstile.HTTPClient = old }
}
...
func TestCreateOrderManualSkipsStockCheck(t *testing.T) {
	...
	// 自动发货：库存不足仍应拦截
	autoPID := fixtures.SeedProductWithCards(t, d, 1)
	if rec := post(map[string]any{
		"product_id": autoPID, "qty": 5, "contact": "buyer@test.com",
		"trade_type": "usdt.trc20", "cf-turnstile-response": "tok",
	}); rec.Code != http.StatusBadGateway || !strings.Contains(rec.Body.String(), "库存不足") {
		t.Fatalf("auto out-of-stock status=%d body=%s, want 502 库存不足", rec.Code, rec.Body.String())
	}
}
...
// TestCreateOrderWithoutTurnstileConfig 未配置 Turnstile 时下单不应被 403 拦截
// （回归：下单接口曾无条件调用 siteverify，导致"没配人机验证就下不了单"）。
func TestCreateOrderWithoutTurnstileConfig(t *testing.T) {
	s, d := newCallbackServer(t)
	store := settingssqlite.NewStore(d)
	// 不写 turnstile_secret：模拟未配置场景。
	gw := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"status_code":200,"data":{"trade_id":"T1","payment_url":"https://pay.test/1"}}`))
	}))
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("order without turnstile status=%d body=%s, want 200", rec.Code, rec.Body.String())
	}
}
