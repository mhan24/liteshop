...
func signedCallbackPayload(t *testing.T, token string, params map[string]string) []byte {
	...
	params["signature"] = payment.Sign(sigParams, token)
...
func TestPaymentCallbackHTTP(t *testing.T) {
	...
	orderNo := testutil.SeedOrder(t, d, pid, models.OrderWaitingPayment, "T1")
...
func TestPaymentCallbackCancelHTTP(t *testing.T) {
	...
	pid := testutil.SeedProductWithCards(t, d, 2)
	orderNo := testutil.SeedOrder(t, d, pid, models.OrderWaitingPayment, "T2")
	...
	testutil.WaitFor(t, 3*time.Second, func() bool { return cancelHits.Load() >= 1 }, "gateway cancel-transaction call")
}
...
func TestPaymentCallbackBadSignature(t *testing.T) {
	...
	orderNo := testutil.SeedOrder(t, d, pid, models.OrderWaitingPayment, "T3")
