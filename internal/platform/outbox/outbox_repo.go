...
// EncodeEnvelope 构造 outbox 载荷信封（{"type":...,"version":...,"data":...}）。
// 平台层不感知领域事件语义，事件类型与版本由业务模块提供。
func EncodeEnvelope(eventType string, version int, data any) (string, error) {
	raw, err := json.Marshal(struct {
		Type    string `json:"type"`
		Version int    `json:"version"`
		Data    any    `json:"data"`
	}{Type: eventType, Version: version, Data: data})
	if err != nil {
		return "", err
	}
