...
func TestAllMigrationsRecorded(t *testing.T) {
	d := openTestDB(t)
	names, err := listMigrationFiles(MigrationsRoot())
	if err != nil {
...
func TestMigrationRecognizesLegacyPathVersion(t *testing.T) {
	d := openTestDB(t)
	if _, err := d.Exec(`UPDATE schema_migrations SET version = '/opt/liteshop/migrations/001_init.sql' WHERE version = '001_init.sql'`); err != nil {
		t.Fatalf("rewrite legacy version: %v", err)
	}
	applied, err := migrationApplied(d, "001_init.sql")
	if err != nil {
		t.Fatalf("migrationApplied: %v", err)
	}
	if !applied {
		t.Fatal("absolute legacy migration path must be recognized")
	}
}

func TestMigrationWithLegacyPathRecordsIsIdempotent(t *testing.T) {
	path := filepath.Join(t.TempDir(), "legacy-path.db")
	...
	if _, err := d.Exec(`UPDATE schema_migrations SET version = 'migrations/' || version`); err != nil {
		t.Fatalf("rewrite versions: %v", err)
	}
	if err := Migrate(d); err != nil {
		t.Fatalf("migrate with legacy path records: %v", err)
	}
	var count int
	...
	d.Close()
	files, err := listMigrationFiles(MigrationsRoot())
	if err != nil {
		t.Fatalf("list migrations: %v", err)
	}
	if count != len(files) {
...
func TestMigration007SQLIntegrity(t *testing.T) {
	data, err := os.ReadFile(filepath.Join(MigrationsRoot(), "007_coupon_unique_and_backfill.sql"))
	if err != nil {
