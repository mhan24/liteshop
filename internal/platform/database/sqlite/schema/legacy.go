...
func backfillOrderStatuses(db *sql.DB) error {
	migrations := []struct{ from, to orderdomain.Status }{
		{"pending", orderdomain.OrderWaitingPayment},
		{"failed", orderdomain.OrderPaymentFailed},
	}
	for _, m := range migrations {
		// 数据修复必须验证影响行数：先统计待迁移行，再确认 UPDATE 恰好迁移这些行。
		var matched int
		if err := db.QueryRow(`SELECT COUNT(1) FROM orders WHERE status = ?`, string(m.from)).Scan(&matched); err != nil {
			return err
		}
		res, err := db.Exec(`UPDATE orders SET status = ?, updated_at = ? WHERE status = ?`, m.to, clock.Now(), string(m.from))
		if err != nil {
			return err
		}
		...
		if affected != int64(matched) {
			return fmt.Errorf("backfill %s -> %s: affected %d, expected %d", m.from, m.to, affected, matched)
		}
	}
	return nil
}
...
func ensureSecretsTable(db *sql.DB) error {
	cipher := security.NewCipher(settingssqlite.EnsureSessionSecret(db))
	if cipher == nil {
		return errors.New("cipher init failed")
	}
	for _, key := range settingssqlite.SecretSettingKeys {
		v, err := settingssqlite.GetSetting(db, key)
		if err != nil {
			return err
		}
		...
		if cipher.IsEncrypted(v) {
			if err := settingssqlite.UpsertSecretRaw(db, key, v); err != nil {
				return err
			}
		} else {
			...
			if err := settingssqlite.UpsertSecretRaw(db, key, enc); err != nil {
				return err
			}
		}
