...
// Service 数据库备份用例：VACUUM INTO 一致性快照 + 完整性校验 + 保留策略。
type Service struct {
	DatabasePath string
	Keep         int
}

func NewService(databasePath string, keep int) *Service {
	return &Service{DatabasePath: databasePath, Keep: keep}
}
...
func (s *Service) Run() error {
	...
	d, err := sql.Open("sqlite", fmt.Sprintf("file:%s?_pragma=busy_timeout(5000)", s.DatabasePath))
	if err != nil {
