...
func TestSchedulerRunsJobs(t *testing.T) {
	s := NewScheduler()
	var n atomic.Int32
	s.Add("test", 20*time.Millisecond, false, func(context.Context) error { n.Add(1); return nil })
...
func TestSchedulerRunOnStart(t *testing.T) {
	s := NewScheduler()
	var n atomic.Int32
	s.Add("startup", time.Hour, true, func(context.Context) error { n.Add(1); return nil })
	s.Add("periodic", time.Hour, false, func(context.Context) error { n.Add(1); return nil })
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	s.Start(ctx)
...
func TestSchedulerInvalidIntervalSkipped(t *testing.T) {
	s := NewScheduler()
	var n atomic.Int32
	s.Add("bad", 0, true, func(context.Context) error { n.Add(1); return nil })
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	s.Start(ctx)
...
func TestSchedulerRecordsRuns(t *testing.T) {
	...
	s.Add("ok-job", time.Hour, true, func(context.Context) error { return nil })
	s.Add("err-job", time.Hour, true, func(context.Context) error { return errors.New("boom") })
	s.Add("panic-job", time.Hour, true, func(context.Context) error { panic("boom-panic") })
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	s.Start(ctx)
...
func TestBackupJob(t *testing.T) {
	...
	d.Close()
	run := backup.NewService(path, 1).Run
	run()
	backups := dir + "/backups"
