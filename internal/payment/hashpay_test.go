...
func TestHashPayVerifyCallback(t *testing.T) {
	...
	params, err := gw.VerifyCallback(env)
	if err != nil {
		t.Fatalf("verify callback: %v", err)
	}
	if params["order_id"] != "ORD-10001" {
		t.Fatalf("order_id = %q, want ORD-10001", params["order_id"])
	}
	if params["trade_id"] != "hp-123" {
		t.Fatalf("trade_id = %q, want hp-123", params["trade_id"])
	}
	if params["status"] != "paid" {
		t.Fatalf("status = %q, want paid", params["status"])
	}
	if params["block_transaction_id"] != "0xabc" {
		t.Fatalf("block_transaction_id = %q, want 0xabc", params["block_transaction_id"])
	}
}
...
func TestHashPayVerifyCallbackErrors(t *testing.T) {
	...
	if _, err := empty.VerifyCallback([]byte(`{}`)); err != ErrGatewayNotConfigured {
...
func TestHashPayCreateTransaction(t *testing.T) {
	...
	url, tradeID, err := gw.CreateTransaction(CreateInput{
		OrderID:     "ORD-10001",
		Amount:      12.34,
		Fiat:        "USD",
