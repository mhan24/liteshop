...
func (r *ProductRepository) Update(p models.Product, id int64) error {
	_, err := r.db.Exec(`UPDATE products SET name = ?, description = ?, image_url = ?, price_cents = ?, status = ?, category = ?, sort_order = ?, is_pinned = ?, faq = ?, wholesale = ?, min_qty = ?, max_qty = ?, cost_cents = ?, delivery_type = ?, updated_at = ? WHERE id = ?`,
		p.Name, p.Description, p.ImageURL, p.PriceCents, p.Status, p.Category, p.SortOrder, p.IsPinned, faqJSON(p.FAQ), wholesaleJSON(p.Wholesale), p.MinQty, p.MaxQty, p.CostCents, normalizeDeliveryType(p.DeliveryType), models.Now(), id)
	return err
}
