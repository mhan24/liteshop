...
import (
	...
	"shop/internal/modules/coupon/domain"
	"shop/internal/shared/clock"
)

// CouponRepository 实现 coupon/application.CouponStore。
...
var (
	ErrCouponNotFound      = domain.ErrCouponNotFound
	ErrCouponExpired       = domain.ErrCouponExpired
	ErrCouponUsedUp        = domain.ErrCouponUsedUp
	ErrCouponNotApplicable = domain.ErrCouponNotApplicable
)

// GetCouponByCode 按券码查询优惠券（含有效期/启用检查）。
func (r *CouponRepository) GetCouponByCode(code string) (domain.Coupon, error) {
	var c domain.Coupon
	...
	if c.ExpiresAt > 0 && clock.Now() > c.ExpiresAt {
...
// 返回抵扣金额（分）与校验错误。
func (r *CouponRepository) ApplyCoupon(code string, amountCents int64, productID int64) (int64, error) {
...
func (r *CouponRepository) UseCoupon(couponID int64, orderNo string, discountCents int64) error {
	tx, err := r.db.Begin()
	if err != nil {
		return err
	}
	defer tx.Rollback()
	// 原子校验 + 递增，防止并发超限
	res, err := tx.Exec(`UPDATE coupons SET used_count = used_count + 1, updated_at = ? WHERE id = ? AND (max_uses = 0 OR used_count < max_uses)`, clock.Now(), couponID)
	if err != nil {
		return err
	}
	...
	if _, err := tx.Exec(`INSERT INTO coupon_usages(coupon_id, order_no, discount_cents, created_at) VALUES(?, ?, ?, ?)`, couponID, orderNo, discountCents, clock.Now()); err != nil {
		return err
	}
	return tx.Commit()
}
...
func (r *CouponRepository) RefundByOrderNo(orderNo string) (bool, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return false, err
	}
	defer tx.Rollback()
	refunded, err := RefundByOrderNoTx(tx, orderNo)
	if err != nil {
		return false, err
	}
...
func RefundByOrderNoTx(tx *sql.Tx, orderNo string) (bool, error) {
	// 找到该订单使用的券
...
func (r *CouponRepository) GetCouponIDByCode(code string) (int64, error) {
	var id int64
...
// ListCoupons 返回全部优惠券。
func (r *CouponRepository) ListCoupons() ([]domain.Coupon, error) {
...
// CreateCoupon 新建优惠券。
func (r *CouponRepository) CreateCoupon(c domain.Coupon) error {
	now := clock.Now()
...
// UpdateCoupon 更新优惠券。
func (r *CouponRepository) UpdateCoupon(c domain.Coupon) error {
	_, err := r.db.Exec(`UPDATE coupons SET code = ?, type = ?, value_cents = ?, percent = ?, min_amount_cents = ?, max_uses = ?, product_id = ?, active = ?, expires_at = ?, updated_at = ? WHERE id = ?`,
		c.Code, c.Type, c.ValueCents, c.Percent, c.MinAmountCents, c.MaxUses, c.ProductID, boolInt(c.Active), c.ExpiresAt, clock.Now(), c.ID)
	if isUniqueViolation(err) {
		return domain.ErrCouponExists
	}
	return err
}

// DeleteCoupon 删除优惠券。
func (r *CouponRepository) DeleteCoupon(id int64) error {
