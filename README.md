...
English version: [README.en.md](README.en.md) / 英文版见 [README.en.md](README.en.md)
...
> 兼容说明：存量订单（查看令牌功能上线前创建）仍支持"邮箱 + 订单号"访问卡密，属过渡兼容、计划后续下线；新订单一律使用随订单邮件发送的查看令牌（令牌只发往登记邮箱）。邮箱查询接口不返回订单号与查看链接，所有订单的访问链接统一通过邮件下发。
- 隐私 / 服务条款 / 首次初始化 `/setup`
...
> 运行时配置（站点地址、支付、通知等）全部在初始化 `/setup` 与管理后台写入数据库 `settings` 表，不读取任何应用级环境变量。
