...
func newOrderService(t *testing.T) (*orderapp.OrderService, *inventorysqlite.KeyRepository, *fixtures.MockGateway, *fixtures.NotifyRecorder, *sql.DB, int64) {
	t.Helper()
...
func testProduct(pid int64) productdomain.Product {
	return productdomain.Product{ID: pid, Name: "集成测试商品", PriceCents: 1000, MinQty: 1, MaxQty: 10, Status: "active"}
}
...
func TestPaymentCallbackAndDuplicate(t *testing.T) {
	...
	if err != nil || o.Status != orderdomain.OrderWaitingPayment {
		t.Fatalf("order status = %v (%v), want waiting_payment", o.Status, err)
	}
	// 创建事件通知（异步）
	fixtures.WaitFor(t, 2*time.Second, func() bool { return rec.CreatedCount() == 1 }, "order_created notify")
	...
	pending, err := outbox.FetchOutboxEvents(d, 10)
	if err != nil {
	...
	if err := outbox.NewOutboxService(d).PublishPending(context.Background(), func(payload string) error {
		e, derr := orderapp.DecodeEvent(payload)
		if derr != nil {
			return derr
		}
		rec.Publish(e)
	...
	if err != nil || o2.Status != orderdomain.OrderDelivered {
		t.Fatalf("order status = %v (%v), want delivered", o2.Status, err)
	}
	if o2.PaymentStatus != orderdomain.PaymentConfirmed {
		t.Fatalf("payment status = %q, want confirmed", o2.PaymentStatus)
	}
	fixtures.WaitFor(t, 2*time.Second, func() bool { return rec.PaidCount() == 1 }, "send paid notify")
	...
	pending, _ = outbox.FetchOutboxEvents(d, 10)
	if len(pending) != 0 {
	...
	if o3.PaymentStatus != orderdomain.PaymentConfirmed {
		t.Fatalf("payment status after duplicate = %q, want confirmed", o3.PaymentStatus)
	}
	// 重复回调后再次运行 outbox：无新增事件、通知不重复。
	_ = outbox.NewOutboxService(d).PublishPending(context.Background(), func(payload string) error {
		e, derr := orderapp.DecodeEvent(payload)
		if derr != nil {
			return derr
		}
...
func TestCancelOrderReleasesCards(t *testing.T) {
	...
	o, err = svc.GetOrderByNo(orderNo)
	if err != nil || o.Status != orderdomain.OrderCancelled {
		t.Fatalf("order status = %v (%v), want cancelled", o.Status, err)
	}
	if o.PaymentStatus != orderdomain.PaymentCancelled {
	...
	fixtures.WaitFor(t, 2*time.Second, func() bool { return gw.CancelCount() == 1 }, "gateway cancel call")
}
...
func TestExpireStaleClosesTimeoutOrders(t *testing.T) {
	...
	o, err := svc.GetOrderByNo(orderNo)
	if err != nil || o.Status != orderdomain.OrderExpired {
...
func TestPaymentCreateFailureReleasesCards(t *testing.T) {
	...
	o, err := svc.GetOrderByNo(orderNo)
	if err != nil || o.Status != orderdomain.OrderPaymentFailed {
		t.Fatalf("order status = %v (%v), want payment_failed", o.Status, err)
	}
	if o.PaymentStatus != orderdomain.PaymentFailed {
...
func TestCallbackAfterCancelNoEffect(t *testing.T) {
	...
	if o2.Status != orderdomain.OrderCancelled {
...
func TestConcurrentLastCardPurchase(t *testing.T) {
	for iter := 0; iter < 5; iter++ {
		...
		pid := fixtures.SeedProductWithCards(t, d, 1) // 最后一张
		p := testProduct(pid)
