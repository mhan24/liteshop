...
func TestCreateOrderGatewayChoice(t *testing.T) {
	...
	svc := orderapp.NewOrderService(orderRepo, func(gateway string) orderapp.PaymentGateway {
		requested = append(requested, gateway)
		return gw
	}, func() settingsdomain.PaymentConfig {
	...
	pid := fixtures.SeedProductWithCards(t, d, 4)
